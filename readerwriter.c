...
#define SEMNAME1 "/mysemaphore10"
#define SEMNAME2 "/mysemaphor11"
sem_t * q;
sem_t * s;
...
int main() {
	s = sem_open(SEMNAME1, O_CREAT, S_IRUSR | S_IWUSR, 1); //escritor 
	q = sem_open(SEMNAME2, O_CREAT, S_IRUSR | S_IWUSR, 1); //mutex lectores 
...
void *reading(void* data) {
	...
	long _tiempo;
	_tiempo = __waitt__;
	sem_wait(s); //mutex -> s    roomEmpty --> q
	contador++;

	if (contador ==1){
		sem_wait(q); //El primero en entrear bloquea a cualquier escritor que quiera entrar 
	}
	
	sem_post(s);  //da entrada a lectores 


	printf("El lector %d estara leyendo por %li ms. Hay %d lectores en el salon		          \n",id,_tiempo,contador);
	__reading__(_tiempo); //pasar tiempo
	
	sem_wait(s); // baja contador para entrada 
	contador--;  //sale 
	if (contador==0){
		sem_post(q); //Libera la sala 
	}
	sem_post(s); 
...
void *writing(void* data) {
	...
	int id = (int) data; // ciclo de ejecución en el que va el programa 1-10
	long _tiempo = __waitt__;
...
void  INThandler(int sig)
{
     ...
     c = getchar();
     if (c == 'y' || c == 'Y') {
 		sem_close(q); sem_unlink(SEMNAME2);
 		sem_close(s); sem_unlink(SEMNAME1);
		exit(0);
     } else
          	signal(SIGINT, INThandler);
